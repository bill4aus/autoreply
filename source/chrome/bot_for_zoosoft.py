...
def get_friends_message(name):
	...
	while 1:
		...
		try:
			...
			if not name in lastsay:
				lastsay[name]=u'测试'
				print('not have key')
				print(lastsay[name])

			if not name in isayto:
				isayto[name]=u'测试'
			...
			# 	# 	ssay='注意：用户-'+nname+' 发照片或者文件，请留意检查相关聊天记录'
				...
				#中文机器人  自有
				# '&group='+urllib.parse.quote(name.encode('utf8'))+
				# +'&username='+urllib.parse.quote(nname.encode('utf8'))
				...
				ress = requests.get(url=urll,params={'role':'doctor','word':ssay,'keshi':'xinggongnengzhangai'}).text
				...
				if ress == 'stop to say||stop to say':
					...
					logging.info('robot will respond with : '+ress)
					# print ress
					try:
